...
- **1956 `தாய்மை` — ACTIVE / EARLIER-WITNESS ANTHOLOGY:** source registered at [`collections/1956-thaaymai/`](collections/1956-thaaymai/README.md); four works mapped; all four already canonical elsewhere; printed pages **80–81 are absent** from the supplied PDF. `தாய்மை` witness is **CLOSED / PASS 16/16**. `ஆட்டக் காவடி` witness is **IN PROGRESS 6/11** through scans **23–28 / printed 21–26**; one canonical-recheck candidate is open (`நாளெல்லாம்` vs current `நாவெல்லாம்`). User batching rule: **10 source pages per iteration**; exact next = scans **29–38**.
