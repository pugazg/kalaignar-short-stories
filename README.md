...
- **1956 `தாய்மை` — ACTIVE / EARLIER-WITNESS ANTHOLOGY:** `தாய்மை` witness **CLOSED / PASS 16/16**; `ஆட்டக் காவடி` witness **CLOSED / PASS 11/11**; cross-repo `நடுத்தெரு நாராயணி` witness in `pugazg/kalaignar-novels` is **IN PROGRESS 25/26** through scans **34–58 / printed 32–56**. It now has four canonical-recheck candidates and one localized 1956-only dialogue line; canonical Tamil/English remain unchanged. Printed pages **80–81** remain absent. User batching rule: **10 source pages per iteration**; exact next = scan **59** to close Narayani, then `அரும்பு` scans **60–68**.
