# Additional source witness — சந்தனக்கிண்ணம் — 1953 first edition

Comparison-only layer for the 1953 `நாடும் நாடகமும்` witness. The **1977 anthology remains canonical**.

## Witness source

- source: `TVA_BOK_0064193_நாடும்_நாடகமும்.pdf`
- publication: **நாடும் நாடகமும்**
- author: **மு. கருணாநிதி**
- publisher: **திராவிடப்பண்ணை**
- represented edition: **முதல் பதிப்பு — 1953**
- witness scans: **37–51**
- witness printed pages: **29–43**
- opening: scan **37**
- ending: scan **51**
- forward boundary: scan **52** opens **ஆலமரத்துப் புறாக்கள்**
- source PDF committed: **No**

## Canonical target

- workspace: `stories/santhana-kinnam/`
- canonical controlling edition: **1977 first-edition anthology**
- canonical scans: **47–56 / printed 38–47**
- canonical state: **CURRENT PASS / CLOSED — 10/10 verified**
- canonical Tamil changed from this witness: **No**
- canonical English changed from this witness: **No**

## Same-work identity

**CONFIRMED.**

Direct comparison of the 1953 opening pages with the existing canonical text confirms the same narrative and wording sequence, including:

- the wedding address opening;
- `திராவிட உட்கல வங்க`;
- `திராவிடர் வரலாறு`;
- the Kamala / Vijaya relationship;
- the symbolic `சந்தனக் கிண்ணம்` gift.

Therefore the 1953 unit is **not a new canonical story**.

## Current state

- routing correction: **COMPLETE**
- duplicate canonical folder: **TO BE REMOVED IN THE SAME CORRECTIVE COMMIT**
- witness comparison: **NOT STARTED**
- witness scans compared: **0/15**
- material variants recorded: **0**
- canonical recheck candidates: **0**
- unresolved witness readings: **0**

## Exact next activity

Begin comparison-only witness review with **scans 37–41 / printed 29–33**.

Compare the 1953 witness directly against the canonical 1977 text. Record wording, morphology, spelling, punctuation, spacing, paragraph/layout, and substantive narrative variants. Do **not** import witness wording into canonical Tamil automatically. If the 1953 witness exposes a possible canonical error, reopen the exact 1977 controlling scan before any canonical repair.

Do not create Stage-1 canonical page records for this witness.
