# விலையால் வாங்கலையோ

Canonical story workspace activated from the 1953 collection **தப்பிவிட்டார்கள்**.

## Canonical source

- source file: `TVA_BOK_0064098_தப்பிவிட்டார்கள்.pdf`
- collection: **தப்பிவிட்டார்கள்**
- author: **மு. கருணாநிதி**
- publisher: **திராவிடன் பதிப்பகம், வேலூர் (வ. ஆ.)**
- edition: **நான்காம் பதிப்பு — ஆகஸ்ட் '53**
- story scans: **24–31**
- printed pages: **22–29**
- physical story pages: **8**
- source type: **image-only**
- source authority: **direct scan pixels**
- source PDF committed: **No**

## Identity / deduplication

A fresh live-main duplicate check was repeated immediately before activation using:

- exact title `விலையால் வாங்கலையோ`;
- plausible transliteration / slug variants;
- the character name `வைரக்கண்ணு`;
- distinctive opening dialogue anchors.

No existing canonical story was found. This workspace is therefore the canonical route for the work.

## Current Tamil workflow

**T1 FIRST-PASS TRANSCRIPTION — COMPLETE 8/8.**

- scans **24–25 / printed 22–23**: T1 COMPLETE / COMMITTED
- scans **26–27 / printed 24–25**: T1 COMPLETE / COMMITTED
- scans **28–29 / printed 26–27**: T1 COMPLETE / COMMITTED
- scans **30–31 / printed 28–29**: T1 COMPLETE / COMMITTED
- scan **32 / printed 30**: boundary witness confirms next story `முந்நூறு ரூபாய்`
- Stage 2 historical-glyph / difficult-reading audit: **8/8 COMPLETE / PASS**
- Stage 3 final source / visual-fidelity audit: not started
- Stage 4 synchronized Tamil assembly / controls: not started
- English: **BLOCKED until Tamil/source closure**

After Stage 2, all **8/8** page records are `needs-review`. No page is `verified` before Stage 3.

## Small-batch plan

To keep every step durable and synchronized:

1. T1 scans 24–25 — **COMPLETE / COMMITTED**
2. T1 scans 26–27 — **COMPLETE / COMMITTED**
3. T1 scans 28–29 — **COMPLETE / COMMITTED**
4. T1 scans 30–31 + confirm scan-32 next-story boundary — **COMPLETE / COMMITTED**
5. Stage 2 glyph audit scans 24–27 — **COMPLETE / PASS**
6. Stage 2 glyph audit scans 28–31 — **COMPLETE / PASS**
7. Stage 3 final source / visual-fidelity audit scans 24–27 — **NEXT**
8. Stage 3 scans 28–31
9. Stage 4 Tamil assembly / controls
10. English only after Tamil/source closure
Current T1 checkpoints: [`T1_BATCH_024_025.md`](T1_BATCH_024_025.md), [`T1_BATCH_026_027.md`](T1_BATCH_026_027.md), [`T1_BATCH_028_029.md`](T1_BATCH_028_029.md), and [`T1_BATCH_030_031.md`](T1_BATCH_030_031.md).

Stage-2 checkpoints: [`STAGE2_BATCH_024_027.md`](STAGE2_BATCH_024_027.md) and [`STAGE2_BATCH_028_031.md`](STAGE2_BATCH_028_031.md).

Stage-2 cumulative repairs: **30 occurrence-level textual repairs**, including **9 historical-character identity corrections / resolutions**.

Current next small task: **Stage 3 final source / visual-fidelity audit scans 24–27 only**.
