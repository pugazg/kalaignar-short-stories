# Additional source witness — `அராபியக் கதை` — 1987

This directory records the 1987 **`கலைஞர் சொன்ன குட்டிக் கதைகள்`** witness for canonical **`ஜாடி குட்டி போடுமா?`**.

- source: `TVA_BOK_0065566_கலைஞர்_சொன்ன_குட்டிக்_கதைகள்_1987.pdf`
- SHA-256: `29c986812f95c43105eec4b38fa9e02c3c4f66cf3c4e8a686b81dd28c1164f0f`
- represented edition: Second Edition, 1987
- 1987 heading: **`அராபியக் கதை`**
- span: **lower scan 6 → upper scan 7**
- identity: **existing canonical story — witness only**
- 1987 page transcription: **2 / 2 verified**
- independent historical-glyph Pass 2: **2 / 2 PASS**
- unresolved 1987 witness text: **0**
- 1987 witness-local English: **PASS** — `translations/en/arabiyak-kathai.md`
- translation review: **PASS** — `TRANSLATION_REVIEW.md`

The controlling canonical 2008 Tamil and English remain unchanged.
