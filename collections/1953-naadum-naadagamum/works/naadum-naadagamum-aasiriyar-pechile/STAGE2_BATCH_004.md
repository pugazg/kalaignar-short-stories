# Stage 2 — Batch 4 visual text-fidelity audit

Work: **நாடும் நாடகமும் (ஆசிரியர் பேசுகிறார்)**  
Controlling source: `TVA_BOK_0064193_நாடும்_நாடகமும்.pdf`  
Batch: scans **20–24 / printed 16, 16-A–16-D**

## Result

- pages audited directly against controlling scan pixels: **5/5**
- Stage 2: **COMPLETE**
- page status after Stage 2: **needs-review 5/5**
- source-supported ordinary-fidelity corrections: **7**
- unresolved source-text spans: **0**
- guessed readings introduced: **0**
- scan 19→20: **PASS / complete paragraph boundary**
- scan 20→21: **PASS / complete paragraph boundary**
- scan 21→22: **PASS — `கொண்டிட` / `வேண்டாம்.`**
- scan 22→23: **PASS — `நல்ல` / `சமூக நாடகங்கள்`**
- scan 23→24: **PASS / complete paragraph boundary**
- scan 24: **work-ending page confirmed**
- scan 25: **boundary witness only — opens `தெருக்கூத்து` / காட்சி 1; excluded**
- Stage 3 historical-glyph audit: **NEXT**
- Stage 4 final independent check: **NOT STARTED**

No OCR, web copy, Wikisource, catalogue text, alternate edition, or contextual reconstruction was used as source authority.

## Source-supported corrections

### Scan 20 / printed 16 — 5

- `நிலைக் கண்ணாடிகள் உடைத்தெறியலாம்` → `நிலைக் கண்ணாடிகளை உடைத்தெறியலாம்`
- first `ஒவ்வொரு துண்டும்` → `ஒவ்வொரு துண்டுமே`
- `எடுக்கும் போது என்ன தோன்றும், என்ன காண்பர் அதிலே.` → `எடுக்கும் போது என்ன தோன்றும். என்ன காண்பர் அதிலே.`
- `இத்துண்டே போதும், இதனையே` → `இத்துண்டே போதும். இதனையே`
- second `சிதறிய ஒவ்வொரு துண்டும்,` → `சிதறிய ஒவ்வொரு துண்டுமே,`

Directly rechecked and retained:

- `நாடகங்கட்குத் தடைவிதித்து`
- `துணிந்து விட்டனர்`
- `எடுக்கப்படும் பலரால்`
- source-visible open quotation before `நமது நிலைமையைக்`
- `ஆணவக்காரர்கள்`
- `இதனையே நானும் காட்டுவேன் மக்களுக்கு என`
- `கல்லெறியும் வேகத்திலே`

### Scan 21 / printed 16-A — 1

- `என் சிறிதுகூடப்` → `ஏன் சிறிதுகூடப்`

Directly rechecked and retained:

- `பொழுதுபோக்கும் எண்ணத்துடனும்`
- `போதிய ஆற்றல்`
- joined `ஆர்வங்காரணமாக`
- `வேதனைச் சித்திரத்தைத் தீட்டி விடுவார்களோ`
- quoted `‘அமைச்சூர்ஸ் நாடகங்கள்’`
- final `கொண்டிட`

### Scan 22 / printed 16-B — 1

- `அக்கரையோ` → `அக்கறையோ`

Directly rechecked and retained:

- opening `வேண்டாம்.`
- `காரியமாற்றும் ஆற்றலையும்`
- `நாடகம் நடிக்க, நடத்த இருக்கும், இருக்க வேண்டிய`
- quote punctuation around `“கூத்தாடி”` / `‘கூத்தாடிப் பிழைப்பு’`
- `நாடகத்தில்?`
- source-visible `கொட்டியா`
- `பேச்சு மூலப் பரப்பி`
- final `நல்ல`

### Scan 23 / printed 16-C — 0

No ordinary-text correction required.

Directly rechecked and retained:

- `ஒரு சில ஆண்டுகட்கு முன்னர்`
- `மறுமலர்ச்சி நாடகம் என்றாலே`
- no terminal punctuation after `செய்தும் வருகிறார்`
- `நல்லதொரு எதை யெனத் எண்ணிக் களிப்புறுகிறது தெரியுமா?`
- `பாத்திரத்திற் கேற்ப`

### Scan 24 / printed 16-D — 0

No ordinary-text correction required.

Directly rechecked and retained:

- `அறிவை யகற்றி`
- `வாழா வெட்டிகளாக`
- `அஞ்ஞானக் கருத்துக்களின்மீது`
- `ஆடைகளை யணிகிறோம்`
- `கிடைத்ததை யெல்லாம்`
- `பயன் படுத்துகிறோம்`
- `‘கூத்தாடி, கூத்தாடி’,`
- final `என்றுதான்`

## Boundary result

All physical boundaries pass:

- scan 19→20 — no split
- scan 20→21 — no split
- scan 21→22 — `கொண்டிட` / `வேண்டாம்.`
- scan 22→23 — `நல்ல` / `சமூக நாடகங்கள்`
- scan 23→24 — no split
- scan 24 — prose/discourse work-ending
- scan 25 — separate `தெருக்கூத்து` opening; excluded

No duplicated or omitted text was found at the joins.

## Status

- Stage 1: **COMPLETE 5/5**
- Stage 2: **COMPLETE 5/5**
- Stage-2 corrections: **7**
- unresolved source-text spans: **0**
- pages: **needs-review 5/5**
- guessed readings: **0**
- Stage 3: **NEXT**
- Stage 4: **NOT STARTED**

## Exact next activity

Run **Batch 4 Stage 3 historical Tamil glyph audit** for scans **20–24 / printed 16, 16-A–16-D**.

Explicitly audit the mandatory historical-glyph families:

`ணா / ணை / ணொ / ணோ / லை / ளை / றா / றொ / றோ / னா / னை / னொ / னோ`.

Keep all five pages `needs-review`, synchronize controls, commit Stage 3, and stop before Stage 4 or `தெருக்கூத்து`.
