# கலைஞர் சொன்ன கதைகள் — 2008 second-edition anthology source

This folder registers **`கலைஞர் சொன்ன கதைகள்`** as a collection-level archival source for `pugazg/kalaignar-short-stories`.

The book is a physical collection/source container, not one canonical story. Its forty numbered stories/anecdotes are activated under canonical `stories/<slug>/` workspaces. Every story received its own live-main duplicate check and direct source verification.

## Source snapshot

- source filename: `TVA_BOK_0065857_கலைஞர்_சொன்ன_கதைகள்.pdf`
- printed title: **கலைஞர் சொன்ன கதைகள்**
- printed author line: **டாக்டர் கலைஞர் மு. கருணாநிதி**
- publisher: **பாரதி பதிப்பகம்**, 126/108, உஸ்மான் சாலை, தி.நகர், சென்னை-600017
- first edition: **August 2004**
- registered scanned edition: **Second Edition: December 2008**
- PDF scans: **82**
- printed story pages: **7–79**
- stories in printed contents: **40**
- story-text scans: **9–81**
- scan **82**: verified back cover
- source PDF committed to GitHub: **No**

Full checksum, byte size, imprint and scan-condition notes are in `metadata/source.md`.

## Intake / processing state

- printed contents transcribed: **40 / 40**
- calculated TOC-derived ranges: **40 / 40**
- story-opening scans visually checked: **40 / 40**
- final Story-40/back-cover boundary checked: **Yes**
- canonical story workspaces activated from this collection: **40 / 40**
- Tamil source processing complete: **40 / 40**
- Tamil source processing pending: **0 / 40**
- word-by-word text fidelity complete: **40 / 40**
- visual fidelity complete: **40 / 40**
- completed-story blocked / unresolved story text: **0**
- English translation complete from this collection: **25 / 40**
- English translation pending: **15 / 40**
- English translation `NEEDS REVIEW`: **0**
- English translation tracker: [`ENGLISH_TRANSLATION_PROGRESS.md`](ENGLISH_TRANSLATION_PROGRESS.md)

**Tamil source pass: COMPLETE.** The source closes with Story 40 on scan **81 / printed page 79**; scan **82** is the verified physical back cover and contains no further story text.

The story block uses **scan = printed page + 2**, but successive TOC start pages are routing coordinates only. Direct page review confirms repeated **shared physical boundary scans** where one story closes above the next story heading.

## Completed source work — Stories 1–40

| # | Story | Canonical workspace | Verified physical source span | Tamil |
|---:|---|---|---|---|
| 1 | `அப்படித்தான் சிரிப்பேன்` | `stories/appadithan-sirippen/` | scan 9 → top 10 | PASS |
| 2 | TOC `ஐஸ்கட்டி` / opening `ஐஸ் கட்டி` | `stories/ice-katti/` | lower 10 → upper 11 | PASS |
| 3 | `தலையில் மலை` | `stories/thalaiyil-malai/` | lower 11 → upper 16 | PASS |
| 4 | `வெறும் கை முழம் போடும்` | `stories/verum-kai-muzham-podum/` | lower 16 → upper 17 | PASS |
| 5 | `கூட்டணி` | `stories/koottani/` | lower 17 → upper 18 | PASS |
| 6 | `சீற வேண்டாமா?` | `stories/seera-vendama/` | lower 18 → upper 19 | PASS |
| 7 | `கழுதையின் கதை` | `stories/kazhuthaiyin-kathai/` | lower 19 → 20 → upper 21 | PASS |
| 8 | `உனக்கு வயதென்ன?` | `stories/unakku-vayathenna/` | lower 21 → upper 22 | PASS |
| 9 | `தமிழன் என்று சொல்லடா!` | `stories/thamizan-endru-sollada/` | lower 22 → 23–24 → upper 25 | PASS |
| 10 | `கடமை கண்ணியம் கட்டுப்பாடு` | `stories/kadamai-kanniyam-kattuppadu/` | lower 25 → 26 → upper 27 | PASS |
| 11 | TOC `சாவிதான் இல்லை` / opening `சாவி தான் இல்லை` | `stories/saavi-thaan-illai/` | lower 27 → 28 | PASS |
| 12 | `கண்ணில் கால்` | `stories/kannil-kaal/` | 29 → upper 30 | PASS |
| 13 | `மயில் ராவணன்` | `stories/mayil-ravanan/` | lower 30 → 31 | PASS |
| 14 | `ஜாடி குட்டி போடுமா?` | `stories/jaadi-kutti-poduma/` | 32 → upper 33 | PASS |
| 15 | `ஒண்ணு குடுமா?` | `stories/onnu-kuduma/` | lower 33 → 34 → upper 35 | PASS |
| 16 | `அத்திரி பாச்சா` | `stories/aththiri-paachaa/` | lower 35 → upper 36 | PASS |
| 17 | `செருப்போடு இரு` | `stories/seruppodu-iru/` | lower 36 → upper 37 | PASS |
| 18 | `இடிக்குப் பின் மழை` | `stories/idikkup-pin-mazhai/` | lower 37 → 38 → upper 39 | PASS |
| 19 | `நடக்குமா நடக்காதா?` | `stories/nadakkuma-nadakkatha/` | lower 39 → 40–41 → upper 42 | PASS |
| 20 | `கனியும் கணையும்` | `stories/kaniyum-kanaiyum/` | lower 42 → upper 43 | PASS |
| 21 | `இதயம் பேசுகிறது` | `stories/idhayam-pesugirathu/` | lower 43 → upper 44 | PASS |
| 22 | `புலிவால்` | `stories/pulivaal/` | lower 44 → upper 45 | PASS |
| 23 | `தெரியாத பேச்சு` | `stories/theriyatha-pechu/` | lower 45 → 46 → upper 47 | PASS |
| 24 | TOC `வெண்ணெய் உருகுது வெயிலில்!` / opening `வெண்ணெய் உருகுது வெயிலில்` | `stories/vennai-uruguthu-veyilil/` | lower 47 → 48–53 → upper 54 | PASS |
| 25 | `மாமியார் உடைத்தால் மட்டும் மண்சட்டியா?` | `stories/mamiyar-udaithaal-mattum-manchattiya/` | lower 54 → 55–59 → upper 60 | PASS |
| 26 | `பொறுமைக்கு சான்று` | `stories/porumaikku-saandru/` | lower 60 → upper 61 | PASS |
| 27 | TOC `எடுக்கவோ கோக்கவோ!` / opening `எடுக்கவோ கோக்கவோ` | `stories/edukkavo-kokkavo/` | lower 61 → upper 62 | PASS |
| 28 | TOC `அந்த நாள் வந்திலை...` / opening `அந்த நாள் வந்திலை!` | `stories/antha-naal-vanthilai/` | lower 62 → 63 → upper 64 | PASS |
| 29 | TOC `பனித் துளியில் பனைமரம்` / opening `பனித்துளியில் பனை மரம்` | `stories/panithuliyil-panai-maram/` | lower 64 → upper 65 | PASS |
| 30 | `பாரூர் போல...` | `stories/paarur-pola/` | lower 65 → upper 66 | PASS |
| 31 | `இராமனைப் பற்றி இராமன்` | `stories/iramanai-patri-iraman/` | lower 66 → 67–68 → upper 69 | PASS |
| 32 | `மானும் பெருமானும்` | `stories/maanum-perumaanum/` | lower 69 → 70 → upper 71 | PASS |
| 33 | `எழுச்சிக்கு அடையாளம்` | `stories/ezhuchikku-adaiyaalam/` | lower 71 → upper 72 | PASS |
| 34 | `தலையும் நுனியும்` | `stories/thalaiyum-nuniyum/` | lower 72 → upper 73 | PASS |
| 35 | TOC `தும்... பம்... தீம்... தோம்` / opening `தும் பம் தீம் தோம்` | `stories/thum-pam-theem-thom/` | lower 73 → 74–75 → upper 76 | PASS |
| 36 | TOC `நல்லவழியும் நல்ல வழியும்` / opening `நல்வழியும் நல்ல வழியும்` | `stories/nalvazhiyum-nalla-vazhiyum/` | lower 76 → upper 77 | PASS |
| 37 | `நாக்குத் தமிழ் மணக்கும்` | `stories/naakkuth-tamil-manakkum/` | lower 77 → 78 → upper 79 | PASS |
| 38 | `நீதி தேவதையே!` | `stories/neethi-devathaiye/` | lower 79 → upper 80 | PASS |
| 39 | TOC `நன்றி சொல்லும் நேரம்...` / opening `நன்றி சொல்லும் நேரம்` | `stories/nandri-sollum-neram/` | lower 80 → upper 81 | PASS |
| 40 | `பந்தலிலே பாகற்காய்` | `stories/panthalile-paagarkai/` | lower 81; scan 82 back-cover witness | PASS |

All forty stories have **0 blocked / 0 unresolved story text**, complete Tamil assemblies, source metadata, page maps, audits and persistent human-review queues.

## English translation phase

English translation was opened only after explicit user authorization, following closure of Tamil source, text-fidelity and visual-fidelity work.

- complete: **25 / 40**;
- PASS: **25**;
- pending: **15 / 40**;
- needs review: **0**;
- latest completed iteration: **Stories 16–25 — 10 / 10 PASS**;
- latest completed story: **Story 25 `மாமியார் உடைத்தால் மட்டும் மண்சட்டியா?` — PASS**;
- latest English source span: **lower scan 54 / printed 52 → scans 55–59 → upper scan 60 / printed 58**;
- canonical Tamil changed during Stories 16–25 English work: **No**;
- standing English iteration rule: **10 stories per iteration**;
- next iteration: **Stories 26–35**, beginning with Story 26 `பொறுமைக்கு சான்று` — lower scan **60 / printed 58 → upper scan 61 / printed 59**;
- durable tracker: [`ENGLISH_TRANSLATION_PROGRESS.md`](ENGLISH_TRANSLATION_PROGRESS.md).

Stories **16–25** all have complete English files, `TRANSLATION_REVIEW.md`, synchronized story READMEs, source-significant ending ornaments and verified physical page-boundary alignment. Notable controls include Story 18's Socrates `தலை`→`யிலே` split, Story 22's quoted `நாயர் புடிச்ச`→`புலிவால்` split, Story 24's TOC/opening-title variance and eight-page embedded literary letter, and Story 25's seven-page mythic/political analogy with unusual source forms left unnormalized.

The user has explicitly directed that English work continue in **10-story iterations**. The next iteration is Stories **26–35**; the final iteration will be Stories **36–40**.

## Source-title differences

Nine TOC/opening-heading differences are directly documented:

1. Story 2 — TOC **`ஐஸ்கட்டி`** ↔ opening **`ஐஸ் கட்டி`**;
2. Story 11 — TOC **`சாவிதான் இல்லை`** ↔ opening **`சாவி தான் இல்லை`**;
3. Story 24 — TOC **`வெண்ணெய் உருகுது வெயிலில்!`** ↔ opening **`வெண்ணெய் உருகுது வெயிலில்`**;
4. Story 27 — TOC **`எடுக்கவோ கோக்கவோ!`** ↔ opening **`எடுக்கவோ கோக்கவோ`**;
5. Story 28 — TOC **`அந்த நாள் வந்திலை...`** ↔ opening **`அந்த நாள் வந்திலை!`**;
6. Story 29 — TOC **`பனித் துளியில் பனைமரம்`** ↔ opening **`பனித்துளியில் பனை மரம்`**;
7. Story 35 — TOC **`தும்... பம்... தீம்... தோம்`** ↔ opening **`தும் பம் தீம் தோம்`**;
8. Story 36 — TOC **`நல்லவழியும் நல்ல வழியும்`** ↔ opening **`நல்வழியும் நல்ல வழியும்`**;
9. Story 39 — TOC **`நன்றி சொல்லும் நேரம்...`** ↔ opening **`நன்றி சொல்லும் நேரம்`**.

No title form is silently normalized. `indexes/story-inventory.md` retains both witnesses.

## Source-pass closure

**COMPLETE — 40 / 40 stories source-processed; 0 pending.**

The final source iteration closed Stories **32–40**. Story 39 ends above Story 40 on scan **81**; Story 40 then closes on that same scan. Scan **82** is the verified back cover and contains no further story text.

No additional source story is implied after Story 40. The source, text-fidelity and visual-fidelity phases remain closed. English translation is a separately authorized downstream phase and is **IN PROGRESS — 25 / 40 PASS**; modernization, adaptation, republication and Digital Library onboarding remain outside this authorization.